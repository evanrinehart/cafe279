...
/*
a = -231.336659 -118.231645
b = -228.149397 -129.086618
c = -232.000000 -120.000000
d = -216.000000 -120.000000 
*/
...
void main(){
	...
	if(segmentsCross(a,b,c,d)) printf("yes\n");
	else{ printf("no\n"); }
}
...
void main(){
	...
	struct vec2 a = {-231.336659, -118.231645};
	struct vec2 b = {-228.149397, -129.086618};
	struct vec2 c = {-232.000000, -120.000000};
	struct vec2 d = {-216.000000, -120.000000};
	...
	printf("line P (%lf,%lf) (%lf,%lf)\n", a.x, a.y, b.x, b.y);
	printf("line Q (%lf,%lf) (%lf,%lf)\n", c.x, c.y, d.x, d.y);
	printf("abc P A=%lf B=%lf C=%lf\n", lineP[0], lineP[1], lineP[2]);
	printf("abc Q A=%lf B=%lf C=%lf\n", lineQ[0], lineQ[1], lineQ[2]);

	struct vec2 solution;
	int no = solve2x2(lineP, lineQ, &solution);

	if(no) printf("no solution\n");
	else printf("solution x=%lf y=%lf\n", solution.x, solution.y);

	puts("\n*****\n");

	vec2 crossing;

	if(segmentsIntersection(a,b,c,d,&crossing)){
		printf("cross at %lf %lf\n", crossing.x, crossing.y);
	}
	else{
		printf("no crossing\n");
	}
